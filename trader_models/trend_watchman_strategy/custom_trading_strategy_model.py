...
class custom_trading_strategy_model:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                qmt_account='55009640',qmt_account_type='STOCK',strategy_func='trend_watchman_strategy',name='趋势策略则守望者'):
        '''
        分析模型
        '''
        self.exe=exe
    ...
    def mean_line_models(self,df):
        ...
        if mean_30>mean_60:
            score+=25
        return score
    def trend_watchman_strategy_get_del_buy_sell_data(self):
        '''
        处理交易股票池买入股票
        '''
        ...
        if df.shape[0]>0:
            ...
            trader_df=pd.read_excel(r'{}\{}\趋势策略守望者股票池.xlsx'.format(self.path,self.strategy_func),dtype='object')
            ...
            trader_df.to_excel(r'{}\{}\趋势策略守望者交易股票池.xlsx'.format(self.path,self.strategy_func))
    def trend_watchman_strategy_get_buy_sell_stock(self):
        '''
        获取买卖数据
        '''
        with open(r'{}\自定义股票池策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
    def trend_watchman_strategy(self):
        '''
        和策略名称函数保持一样的
        '''
        with open(r'{}\自定义策略模型分析配置.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
