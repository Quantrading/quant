...
import os
class etf_corpse_picking_strategy:
    ...
    def select_etf_fund_data(self):
        ...
        '''
        print('选择ETF')
        with open(r'{}/etf捡尸策略交易配置.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
    ...
    def get_buy_sell_stock(self):
        ...
        获取买卖数据
        '''
        with open('{}/etf捡尸策略交易配置.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        buy_num=text['买入前N']
