...
import time
from tqdm import tqdm
class convertible_bonds_double_low_strategy:
    ...
    def save_balance(self):
        ...
        保持账户数据
        '''
        with open(r'{}/可转债双低策略设置.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        self.trader.connect()
    ...
    def calculated_double_low(self):
        ...
        with open(r'{}/可转债双低策略设置.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        min_value=text['双低最小值']
        max_value=text['双低最大值']
        ...
        df['双低']=df['价格']+df['转股溢价率']
        df.to_excel(r'{}\原始双低\原始双低.xlsx'.format(self.path ))
        #排序
        df=df.sort_values(by='双低',ascending=True)
        df['双低']=pd.to_numeric(df['双低'])
        df1=df[df['双低']>=min_value]
        df2=df1[df1['双低']<=max_value]
        max_time=text['剩余年限上限']
        min_time=text['剩余年限下限']
        try:
            df3=df2[df2['剩余年限']<=max_time]
            df4=df3[df3['剩余年限']>=min_time]
        except:
            df4=df2
        df4.to_excel(r'{}\双低\双低.xlsx'.format(self.path))
        print('计算双低完成')
    ...
    def get_cov_bond_shape_analysis(self):
        ...
        '''
        print('可转债形态分析')
        df=pd.read_excel(r'{}\双低\双低.xlsx'.format(self.path),dtype='object')
        with open(r'{}/可转债双低策略设置.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        n=text['跌破N日均线卖出']
    ...
    def get_del_qzsh_data(self):
        ...
        '''
        print('剔除强制赎回')
        with open('{}/可转债双低策略设置.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
    ...
    def get_buy_sell_stock(self):
        ...
        sell_value=text['双低平仓']
        hold_limit=text['持有限制']
        df=pd.read_excel(r'持股数据\持股数据.xlsx',dtype='object')
        df['证券代码']=df['证券代码'].astype(str)
        df_yjl=pd.read_excel(r'{}\原始双低\原始双低.xlsx'.format(self.path))
        sd_dict=dict(zip(df_yjl['证券代码'],df_yjl['双低']))
        trend=text['是否开启趋势轮动']
        df1=df[df['可用余额']>=10]
        ...
        if df1.shape[0]>0:
            ...
            for stock in hold_stock_list:
                ...
                if value>=sell_value:
                    print('双低平仓 {} 双低{} 大于 平仓值{}'.format(stock,value,sell_value))   
                    sell_list.append(stock)
                else:
                    print('双低平仓 {} 双低{} 小于 平仓值{}'.format(stock,value,sell_value))   
            #跌出排名卖出N
            rank_df=pd.read_excel(r'{}\双低\双低.xlsx'.format(self.path),dtype='object') 
