...
class xueqie_trader:
    ...
    def save_balance(self):
        ...
        return df
    def get_disk_port_data(self,stock='600031.SH',data_type='askPrice_0'):
        '''
        获取盘口数据
        askPrice_0
        askPrice买入
        ...
        self.qmt_data.subscribe_whole_quote(code_list=[stock])
        disk=self.qmt_data.get_full_tick(code_list=[stock])
        tick=disk[stock]
        keys,index=data_type.split('_')[0],data_type.split('_')[-1]
        return tick[keys][int(index)]
