...
class custom_outside_trading_strategy:
    ...
    def get_del_buy_sell_data(self):
        ...
        处理交易股票池买入股票
        '''
        with open(r'{}\自定义外盘交易策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        limit=text['持股限制']
        ...
        if is_buffer=='是':
            ...
            zdf_list=[]
            print('不开启策略缓冲区**********************')
            trader_df=pd.read_excel(r'自定义交易股票池\自定义交易股票池.xlsx',dtype='object')
            trader_df['证券代码']=trader_df['证券代码'].astype(str)
            df=pd.read_excel(r'持股数据\持股数据.xlsx',dtype='object')
            df1=df[df['股票余额']>=10]
            df1['证券代码']=df1['证券代码'].astype(str)
    ...
    def update_all_data(self):
        ...
        with open(r'{}\自定义外盘交易策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        user_def_text=json.loads(com)
        is_bond_models=user_def_text['是否开启债券模型']
        account_av_buy_amount=user_def_text['账户大概可以买入的股票数量']
        with open(r'分析配置.json',encoding='utf-8') as f:
            com=f.read()
        main_text=json.loads(com)
        special_stock_list=main_text['特殊标的']
        special_cash=main_text['特殊固定交易资金']
        fix_cash=main_text['固定交易资金']
        trader_ratio=int(special_cash/fix_cash)
        print('债券模型交易资金比例{}'.format(trader_ratio))
        if is_bond_models=='是':
            print('开启债券模型************************')
            ...
            buy_df,sell_df=self.get_del_not_trader_stock()
            ...
            if hold_stock.shape[0]>0:
                ...
                hold_stock['国债']=hold_stock['证券代码'].apply(lambda x: '是' if (x) in special_stock_list else '不是')
                bond_hold_stock=hold_stock[hold_stock['国债']=='是']
            else:
                hold_amount=0
                bond_hold_stock=pd.DataFrame()
            #债券模型
            buy_stock_list=[]
            sell_stock_list=[]
            bond_models=custom_bond_trend_rotation_strategy(trader_tool=self.trader_tool,exe=self.exe,tesseract_cmd=self.tesseract_cmd,
                                open_set=self.open_set,qmt_path=self.qmt_path,qmt_account=self.qmt_account,
                                qmt_account_type=self.qmt_account_type)
            bond_buy_df,bond_sell_df=bond_models.update_all_data()
            if buy_df.shape[0]>0:
                stock_list=buy_df['证券代码'].tolist()
                for stock in stock_list:
                    buy_stock_list.append(stock)
            if bond_buy_df.shape[0]>0:
                stock_list=bond_buy_df['证券代码'].tolist()
                for stock in stock_list:
                    buy_stock_list.append(stock)
            if sell_df.shape[0]>0:
                stock_list=sell_df['证券代码'].tolist()
                for stock in stock_list:
                    sell_stock_list.append(stock)
            if bond_sell_df.shape[0]>0:
                stock_list=bond_sell_df['证券代码'].tolist()
                for stock in stock_list:
                    sell_stock_list.append(stock)
            buy_stock_list=list(set(buy_stock_list))
            sell_stock_list=list(set(sell_stock_list))
            buy_amount=len(buy_stock_list)
            sell_amount=len(sell_stock_list)
            #账户数量
            account_amount=hold_amount+buy_amount-sell_amount
            if account_amount<account_av_buy_amount:
                print('账户资金充足不用释放资金*************')
                buy_df=pd.DataFrame()
                ...
                print('账户资金不充足释放资金*************')
                #释放数量
                release_amount=account_av_buy_amount-account_amount
                release_amount=abs(release_amount)
                print('释放数量{}********************'.format(release_amount))
                if bond_hold_stock.shape[0]>0:
                    bond_hold_stock['卖出']=bond_hold_stock['证券代码'].apply(lambda x: '是' if str(x) in sell_stock_list else '不是')
                    bond_hold_stock=bond_hold_stock[bond_hold_stock['卖出']=='不是']
                    bond_hold_stock=bond_hold_stock[:int(release_amount/trader_ratio)]
                    bond_stock_list=bond_hold_stock['证券代码'].tolist()
                    print('释放的股票池***************')
                    print(bond_hold_stock)
                    for stock in bond_stock_list:
                        sell_stock_list.append(stock)
                    sell_stock_list=list(set(sell_stock_list))
                    buy_df=pd.DataFrame()
                    buy_df['证券代码']=buy_stock_list
