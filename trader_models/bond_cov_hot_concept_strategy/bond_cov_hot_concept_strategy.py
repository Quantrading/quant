...
import os
import pandas as pd
class bond_cov_hot_concept_strategy:
    def __init__(self,trader_tool='ths',exe='C:/同花顺软件/同花顺/xiadan.exe',tesseract_cmd='C:/Program Files/Tesseract-OCR/tesseract',
                ...
                qmt_account='55009640',qmt_account_type='STOCK',name='run_bond_cov_hot_concept_strategy'):
        '''
        分析模型
        '''
        self.exe=exe
    ...
    def save_balance(self):
        ...
        保持账户数据
        '''
        with open(r'{}/可转债热门概念趋势策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
        self.trader.connect()
    ...
    def get_all_jsl_data(self):
        ...
        df.to_excel(r'{}\集思录数据\集思录数据.xlsx'.format(self.path))
    ...
    def select_cov_bond_data(self):
        ...
        df['转股溢价率']=pd.to_numeric(df['转股溢价率'])
    ...
    def get_all_ths_hot_concept(self):
        ...
        '''
        print('获取同花顺全部题材')
        ths_concept=self.ths_rq.get_stock_concept_rot_rank()
    ...
    def get_bond_cov_stock_dict(self):
        '''
        获取可转债股票重点
        '''
        df=pd.read_excel(r'{}\集思录数据\集思录数据.xlsx'.format(self.path),dtype='object')
        df['正股代码']=df['正股代码'].apply(lambda x:'0'*(6-len(str(x)))+str(x))
        bond_stock_dict=dict(zip(df['正股代码'].tolist(),df['证券代码'].tolist()))
        stock_list=df['正股代码'].tolist()
        return stock_list,bond_stock_dict
    def select_stocks_including_convertible_bonds(self):
        '''
        选择股票含可转债
        '''
        print('选择股票含可转债')
        stock_list,bond_stock_dict=self.get_bond_cov_stock_dict()
        all_concept_stock=pd.read_excel(r'{}\同花顺概念热门成分股\同花顺概念热门成分股.xlsx'.format(self.path),dtype='object')
        def select_bond(x):
            if x in stock_list:
                bond_stock=bond_stock_dict[x]
                return bond_stock
            else:
                return '不是'
        all_concept_stock['是否包含可转债']=all_concept_stock['代码'].apply(select_bond)
        df=all_concept_stock[all_concept_stock['是否包含可转债'] !='不是']
        df['证券代码']=df['是否包含可转债']
        df.to_excel(r'{}\同花顺热门概念可转债\同花顺热门概念可转债.xlsx'.format(self.path))
        return df
    def select_trader_bond_cov(self):
        '''
        选择交易的可转债
        '''
        with open(r'{}/可转债热门概念趋势策略.json'.format(self.path),encoding='utf-8') as f:
        ...
        text=json.loads(com)
        n=text['热门题材交易前N']
        ths_concept=pd.read_excel(r'{}\同花顺热门概念\同花顺热门概念.xlsx'.format(self.path),dtype='object')
        df=pd.read_excel(r'{}\同花顺热门概念可转债\同花顺热门概念可转债.xlsx'.format(self.path),dtype='object')
        trader_list=ths_concept['概念名称'].tolist()[:n]
        def select_stock(x):
            if x in trader_list:
                return '是'
            else:
                return '不是'
        df['选择可转债交易题材']=df['概念名称'].apply(select_stock)
        df=df[df['选择可转债交易题材']=='是']
        df.to_excel(r'{}\选择可转债交易题材\选择可转债交易题材.xlsx'.format(self.path))
        df2=pd.read_excel(r'{}\选择可转债\选择可转债.xlsx'.format(self.path),dtype='object')
        def select_bond(x):
            if x in df['证券代码'].tolist():
                return '是'
            else:
                return '不是'
        df2['可转债题材选择']=df2['证券代码'].apply(select_bond)
        concept_dict=dict(zip(df['证券代码'].tolist(),df['概念名称'].tolist()))
        concept_rank=dict(zip(df['证券代码'].tolist(),df['排序'].tolist()))
        df2=df2[df2['可转债题材选择']=='是']
        df2['概念名称']=df2['证券代码'].apply(lambda x: concept_dict[x])
        df2['排序']=df2['证券代码'].apply(lambda x: concept_rank[x])
        all_df=pd.DataFrame()
        for name in trader_list:
            df3=df2[df2['概念名称']==name]
            all_df=pd.concat([all_df,df3],ignore_index=True)
        all_df=all_df.sort_values(by='排序',ascending=True)
        all_df.to_excel(r'{}\选择可转债交易题材\选择可转债交易题材.xlsx'.format(self.path))
        return all_df
    #*************************************
    ...
    def get_cov_bond_shape_analysis(self):
        ...
        '''
        print('可转债形态分析')
        df=pd.read_excel(r'{}\选择可转债交易题材\选择可转债交易题材.xlsx'.format(self.path),dtype='object')
        try:
            del df['Unnamed: 0']
        except:
            pass
        ...
        df1=df[df['上影线']=='不是']
        df1=df1[df1['跌破均线']=='不是']
        df1.to_excel(r'{}\分析选择可转债\分析选择可转债.xlsx'.format(self.path))
        return df1
    def get_stock_mean_line_retuen_analysis(self):
        ...
        zdf_list=[]
        max_down_list=[]
        df=pd.read_excel(r'{}\分析选择可转债\分析选择可转债.xlsx'.format(self.path),dtype='object')
        try:
            df['Unnamed: 0']
        except:
            pass
    ...
    def get_del_qzsh_data(self):
        ...
        '''
        print('剔除强制赎回')
        with open('{}/可转债热门概念趋势策略.json'.format(self.path),encoding='utf-8') as f:
            com=f.read()
        text=json.loads(com)
    ...
    def updata_all_data(self):
        ...
        self.select_stocks_including_convertible_bonds()
        self.select_trader_bond_cov()
